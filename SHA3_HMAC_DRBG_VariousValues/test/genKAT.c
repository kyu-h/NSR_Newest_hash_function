...
genKAT_main(void)
{
	...
	for(int i=0; i<4; i++){
		...
		sprintf(inputFileAddress, "HMAC_DRBG_KAT/%s.txt", HashName[i]);
		sprintf(outputFileAddress, "HMAC_DRBG_KAT/%s_rsp.txt", HashName[i]);
...
STATUS_CODES
genShortMsgHash_PR(unsigned int rate, unsigned int capacity, unsigned char delimitedSuffix, unsigned int hashbitlen, unsigned int squeezedOutputLength, const char *inputFileName, const char *outputFileName, const char *description)
{
	FILE *fp_in, *fp_out;
	char str;
